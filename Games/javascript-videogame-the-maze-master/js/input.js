const Input = {
    ...
    configureKeys() {
        ...
        for (let key in this.KEYCODE_MAP) {
            ...
            this.PRESS_BUFFER[key.toLowerCase()] = false
            count++
        }
    },
    ...
    changeKey: function(key, value) {
        if (key == this.KEYCODE_MAP.P && value == 0) {
            ...
            const wasPressed = this.KEY_STATES[key] === 1
            this.KEY_STATES[key] = value
            if (value === 1 && !wasPressed) {
                if (key === this.KEYCODE_MAP.UP) this.PRESS_BUFFER.up = true
                if (key === this.KEYCODE_MAP.DOWN) this.PRESS_BUFFER.down = true
                if (key === this.KEYCODE_MAP.LEFT) this.PRESS_BUFFER.left = true
                if (key === this.KEYCODE_MAP.RIGHT) this.PRESS_BUFFER.right = true
            }
        }
    },
    ...
    consumePressed(key = '') {
        ...
        if (!Object.prototype.hasOwnProperty.call(this.PRESS_BUFFER, key)) return false
        const v = !!this.PRESS_BUFFER[key]
        this.PRESS_BUFFER[key] = false
        return v
    },
